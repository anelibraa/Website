// app/components/PopularAnime.js
"use client";
...
export default function PopularAnime() {
  ...
  useEffect(() => {
    const fetchPopularAnime = async () => {
      ...
      setError(false);
      try {
        // Updated Jikan API endpoint: removed 'limit=10' to get more items
        const response = await fetch("https://api.jikan.moe/v4/top/anime?filter=bypopularity");

        if (!response.ok) {
          if (response.status === 429) {
            console.warn("Jikan API rate limit hit for Popular Anime. Please wait a moment and refresh.");
            setError(true);
            return;
          }
        ...
        if (data && data.data) {
          ...
          console.error("Failed to fetch popular anime data from Jikan:", data);
          setAnimeList([]);
          setError(true);
        }
      } catch (err) {
        console.error("Error fetching popular anime from Jikan:", err);
        setAnimeList([]);
        setError(true);
      } finally {
        setLoading(false);
  ...
  return (
    <section className="mb-8 mt-12">
      {/* No direct title here, as it's typically in the page.js for full pages */}
      {loading ? (
        ...
        <p className="text-highlight-color text-center py-8">
          Failed to load popular anime. (Rate limit? Please wait a moment and refresh.)
        </p>
      ) : animeList.length === 0 ? (
        <p className="text-text-dark text-center py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6">
          {animeList.map((anime) => {
            ...
            return (
              ...
              >
                <div className="relative w-full h-48 sm:h-56"> {/* Adjusted height for grid cards */}
                  <Image
                ...
                <div className="p-3">
                  <h3 className="text-sm font-semibold mb-1 text-text-light line-clamp-2"> {/* Adjusted text size for grid cards */}
                    {title}
                  </h3>
